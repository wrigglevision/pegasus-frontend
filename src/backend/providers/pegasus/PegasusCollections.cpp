...
const QString& single_line(const config::Entry& entry)
{
...
void parse_collection_entry(ParserContext& ctx, const config::Entry& entry)
{
    ...
    switch (ctx.helpers.coll_attribs.at(entry.key)) {
        case CollAttrib::SHORT_NAME:
            ctx.cur_coll->setShortName(single_line(entry));
            break;
        ...
        case CollAttrib::EXTENSIONS:
            filter_group.extensions.append(providers::pegasus::tokenize(single_line(entry).toLower()));
            break;
        ...
        case CollAttrib::REGEX:
            filter_group.regex = single_line(entry);
            break;
        ...
        case CollAttrib::LAUNCH_WORKDIR:
            ctx.cur_coll->launch_workdir = single_line(entry);
            break;
    }
}

/*void parse_file_lines(ParserContext& ctx, const config::Entry& entry)
{
    Q_ASSERT(ctx.cur_game);

    for (int i = 0; i < entry.values.length(); i++) {
        const QString& line = entry.values.at(i);

        const auto tok = utils::tokenize_file_entry(line);
        if (!tok.error_msg.isEmpty()) {
            ctx.print_error(entry.line + i + 1, tok.error_msg);
            continue;
        }
        Q_ASSERT(!tok.parts.isEmpty());

        QString path = tok.parts.constFirst().toString();
        HashMap<QString,QString> props;
        for (int i = 1; i + 1 < tok.parts.count(); i += 2)
            props.emplace(tok.parts.at(i).toString().toLower(), tok.parts.at(i + 1));

        modeldata::GameFile filedata {};

        auto it_name = props.find(QStringLiteral("name"));
        if (it_name != props.cend())
            filedata.name = it_name->second;

        ctx.cur_game->files.emplace(path, std::move(filedata));
    }
}*/

/*QVector<QStringRef> tokenize_comma(ParserContext& ctx, const config::Entry& entry)
{
    auto result = utils::tokenize_comma_list(single_line(entry));

    if (!result.error_msg.isEmpty()) {
        ctx.print_error(entry.line, result.error_msg);
        return {};
    }
...
void parse_game_entry(ParserContext& ctx, const config::Entry& entry)
{
    ...
    switch (ctx.helpers.game_attribs.at(entry.key)) {
        ...
        case GameAttrib::PLAYER_COUNT:
            {
                const auto rx_match = ctx.helpers.rx_count_range.match(single_line(entry));
        ...
        case GameAttrib::RELEASE:
            {
                const auto rx_match = ctx.helpers.rx_date.match(single_line(entry));
                if (!rx_match.hasMatch()) {
        ...
        case GameAttrib::RATING:
            {
                const QString& line = single_line(entry);
        ...
        case GameAttrib::LAUNCH_CMD:
            ctx.cur_game->launch_cmd = single_line(entry);
            break;
        case GameAttrib::LAUNCH_WORKDIR:
            ctx.cur_game->launch_workdir = single_line(entry);
            break;
    }
}
...
bool parse_asset_entry_maybe(ParserContext& ctx, const config::Entry& entry)
{
    ...
    providers::pegasus::add_asset(assets, asset_type, entry.values.first(), ctx.dir_path);
    return true;
}
